...
class VADModel(object):
  ...
  def build(cls, 
    ...
    with open(cls._parameters_file(param_dir)) as f:
      ...
      directories = Directories(parameters["log_dir"],
        parameters["checkpoint_sigmoid_dir"])
  ...
  def restore(cls, 
    ...
    with open(cls._parameters_file(param_dir)) as f:
      ...
      checkpoint_file = tf.train.latest_checkpoint(directories.checkpoint_sigmoid_dir)
  ...
  def __init__(self,
    ...
    **kwargs):
    ...
    with tf.name_scope("Sigmoid_Input"):
    ...
    with tf.name_scope("LSTMRNN_Cost"):
      ...
      tf.summary.scalar('pk_miss', self.pk_miss)
      
      self.pk_falsealarm = tf.reduce_mean(
        tf.cast(self.all_temp_pk_falsealarm, tf.float32))
      tf.summary.scalar('pk_falsealarm', self.pk_falsealarm)
  ...
  def plot_training_losses(cost_history, training_epochs):
    ...
    plt.savefig('plot/losses_sigmoid_'+timestr+'.png', bbox_inches='tight')
    # plt.show()
  ...
  def plot_metrics(metric, training_epochs):
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.savefig('plot/accuracies_sigmoid_'+timestr+'.png')
    # plt.show()
  ...
  def plot_predictions(predictions):
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.savefig('plot/prediction_sigmoid_'+timestr+'.png')
    # plt.show()
  ...
  def plot_targets(targets):
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.savefig('plot/target_sigmoid_'+timestr+'.png')
    # plt.show()
  ...
  def plot_prediction_summary(predictions, ground_truth):
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.savefig('plot/summary_sigmoid_'+timestr+'.png')
    # plt.show()
  ...
  def train(self,
        ...
        for batch_step, (batch_x, batch_y) in enumerate(epoch):
          ...
          if epoch_idx % 100 == 99 and batch_step == 0: # Record execution stats
            ...
            _summary, _train_step, _cost, _prediction_series, _label_predictions, _unshaped_predictions = session.run(
              [self.summary, self.train_step, self.cost, self.predictions, self.label_predictions, self.logits], 
              feed_dict={ 
              self.inputs:batch_x, 
          ...
          if (epoch_idx * self.model_parameters.batch_size + batch_step) % checkpoint_every == 0 or (
            ...
            checkpoint_path = os.path.join(self.directories.checkpoint_sigmoid_dir, 'model.ckpt')
    ...
    #summary.close()
    #logger.info("Saved model in %s " % self.directories.checkpoint_sigmoid_dir)
  ...
  def _write_model_parameters(self, param_dir):
    ...
    "checkpoint_sigmoid_dir" : self.directories.checkpoint_sigmoid_dir,
  ...
  def evaluate(self,
    ...
    _summary, _cost, _all_predictions, _label_predictions = session.run(
      ...
      self.predictions, self.label_predictions],
      feed_dict={
    ...
    out_path = "prediction_sigmoid_soft.csv"
    out_path2 = "prediction_sigmoid_hard.csv"
    ...
    with open(out_path, 'w') as f:
      csv.writer(f).writerows(_soft_predictions)

    with open(out_path2, 'w') as f:
      csv.writer(f).writerows(_hard_predictions)
...
class Directories(object):
    def __init__(self, 
      log_dir, 
      checkpoint_sigmoid_dir):
      """ Encaplsulation of the directories names
      :param log_dir: TensorBoard log directory
      :type string
      :param checkpoint_sigmoid_dir: TensorFlow checkpoint directory
      :type string
      """
      self.log_dir = log_dir
      self.checkpoint_sigmoid_dir = checkpoint_sigmoid_dir
